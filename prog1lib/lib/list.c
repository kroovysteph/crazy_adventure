...
Any l_find(List list, AnyIntAnyToBool predicate, Any state) {
	...
	for (ListNode *node = list->first; node != NULL; node = node->next, i++) {
		// #define VALUE(node) ((Any)(((ListNode*)(node)) + 1))
		if (predicate(VALUE(node), i, state)) {
			return VALUE(node);
		}
	}
	return NULL;
...
static void l_inc(Any element, int index, Any state, Any mapped_element) {
	IntPair *ip = element;
...
static void l_int_pair_i(Any element, int index, Any state, Any mapped_element) {
	IntPair *ip = element;
...
List l_map(List list, AnyIntAnyAnyToVoid f, int mapped_element_size, Any state) {
	...
	for (ListNode *node = list->first; node != NULL; node = node->next, i++) {
		...
		f(node + 1, i, state, mapped_node + 1);
	}
	return result;
}
...
static void l_inc_in_place(Any element, int index, Any state) {
	IntPair *ip = element;
	ip->i += 1;
	ip->j += 1;
}
...
static void l_sum_j(Any state, Any element, int index) {
	int *s = state;
	IntPair *ip = element;
	*s += ip->j;
}
...
static void l_minus_j(Any element, Any state, int index) {
	int *s = state;
	IntPair *ip = element;
	*s = ip->j - *s;
}
...
static bool l_eq2(Any element, int index, Any state) {
	IntPair *ip = element;
	return ip->i == 2;
}
